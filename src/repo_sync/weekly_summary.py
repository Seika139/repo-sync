...
def _count_runs(lines: list[str]) -> tuple[int, int]:
    ...
    successes = sum(1 for line in lines if "Finished repo-sync.service" in line)
    failures = sum(1 for line in lines if "repo-sync.service: Failed with result" in line)
